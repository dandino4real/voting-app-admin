...
const useCheckLinkActive = (
  ...
  const pathnameArr = pathname.split('/').splice(0, hrefArr.length);

  if (asArr) {
    return checkEqual(asArr, pathnameArr) && checkEqual(hrefArr, pathnameArr);
  }

  return checkEqual(hrefArr, pathnameArr);
};
